...
--SELECT ('POLYGON(('+@geog1+', '+@geog2+', '+@geog3+', ' +@geog4+', '+@geog1+'))')

UPDATE  Marsh 
SET Marsh.Plg = geography::STGeomFromText('POLYGON(('+@geog4+', '+@geog3+', '+@geog2+', ' +@geog1+', '+@geog4+'))', 4923)
WHERE (Marsh.ID = @I)AND(ID_KA=7) 
SET @I =@I + 1;
END;